...
func Run(binary string, testCase Case) (Result, error) {
	...
	root, err := os.MkdirTemp("", "symbrowse-port-")
	if err != nil {
	...
	command.Env, err = isolatedEnv(home, tmp, runtimeDir, state, clock, seed, testCase.Env, replacements)
	if err != nil {
		return Result{}, err
	}
...
func isolatedEnv(home, tmp, runtimeDir, state, clock, seed string, extra map[string]string, replacements map[string]string) ([]string, error) {
	env := []string{
		"HOME=" + home,
		"USERPROFILE=" + home,
		...
		"SYMBROWSE_PORT_CLOCK=" + clock,
		"SYMBROWSE_PORT_SEED=" + seed,
	}
	...
	for key := range extra {
		...
		if reservedSandboxEnv(upperKey) {
			return nil, fmt.Errorf("case environment cannot override sandbox variable %q", key)
		}
		if !allowedCaseEnv(upperKey) {
			return nil, fmt.Errorf("case environment variable %q is not allowlisted", key)
		}
...
func allowedCaseEnv(key string) bool {
	if strings.HasPrefix(key, "SYMBROWSE_") || strings.HasPrefix(key, "PORT_") {
		return true
	}
	switch key {
